...
public class Main {
    ...
    private static Mahasiswa cariMahasiswa(ArrayList<Mahasiswa> list, String nim){
        Mahasiswa result;
        Collections.sort(list); // mengurutkan list mahasiswa
        // mencari index data yang sesuai dengan input dari user
        int index = Collections.binarySearch(list,new Mahasiswa(nim,null,null));
        result = list.get(index); // mencetak data yang sudah ditemukan
        return result; // return value data
    }
